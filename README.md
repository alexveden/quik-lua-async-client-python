...
- Клиент поке **не поддерживает** подписку на события
...
- Клиент поддерживает проверку наличия соенинения через heartbeat и выдает ошибку если socket QuikLua недоступен
...
- У меня вроде даже работает на Linux Debian с квиком под Wine! Если что, то можно приконнектиться к квику на 
  виртуальной машине.
...
5. Можно использовать сколько угодно паралельных запросов (напр. 1000 одновременно), клиент сбалансирует нагрузку, однако
по дефолту устанавлитается только 5 соединений, я не увидил прироста от большего числа, зато стабильность страдала. 
...
Версия: ранная альфа 0.0.0.1, в разработке.